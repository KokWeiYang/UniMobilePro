...
class _MyAppState extends State<MyApp> {
  ...
  double resultABSz = 0.0;
  int _value = 0;
  int _hvalue = 0;
  int _wvalue = 0;
  int _wcvalue = 0;
  String mortalityr = "none";
                                  ...
                                  child: Row(
                                    children: [
                                      Container(
                                        width: 51,
                                        child: TextField(
                                          controller: numcTextEditingController,
                                          keyboardType: TextInputType.number,
                                          textAlignVertical:
                                              TextAlignVertical.bottom,
                                          textAlign: TextAlign.center,
                          ...
                          child: Row(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: TextButton(
                                    style: ButtonStyle(
                                      overlayColor: MaterialStateProperty.all(Colors.green[300]),
  ...
  void clearMe() {
    numaTextEditingController.clear();
    numbTextEditingController.clear();
    numcTextEditingController.clear();
    numdTextEditingController.clear();
    setState(() {
      resultABSI = 0.0;
      resultABSz = 0.0;
      mortalityr = "none";
  ...
  void calculateABSI() {
    setState(() {
      ...
      double weight = 0.00;
      double waistc = 0.00;
      int age = (int.parse(numaTextEditingController.text)) - 2;

      if (_hvalue == 0) {
        height = (double.parse(numbTextEditingController.text)) / 100;
      } else if (_hvalue == 1) {
        height = (double.parse(numbTextEditingController.text)) * 0.0254;
      }
      if (_wvalue == 0) {
        weight = double.parse(numcTextEditingController.text);
      } else if (_wvalue == 1) {
        weight = double.parse(numcTextEditingController.text) * 0.45359237;
      }
      if (_wcvalue == 0) {
        waistc = (double.parse(numdTextEditingController.text)) / 100;
      } else if (_wcvalue == 1) {
        waistc = (double.parse(numdTextEditingController.text)) * 0.0254;
      }

      bmi = weight / (pow(height, 2));
      resultABSI = waistc / ((pow(bmi, 2 / 3)) * (pow(height, 1 / 2)));

      double absimean = normals[age][_value][1];
      double absisd = normals[age][_value][2];
      resultABSz = (resultABSI - absimean) / absisd;

      if (resultABSz < -0.868) {
        mortalityr = "very low";
      } else if ((resultABSz >= -0.868) && (resultABSz <= -0.272)) {
        mortalityr = "low";
      } else if ((resultABSz >= -0.271) && (resultABSz <= 0.229)) {
        mortalityr = "average";
      } else if ((resultABSz >= 0.230) && (resultABSz <= 0.798)) {
        mortalityr = "high";
      } else {
        mortalityr = "Very high";
      }
    });
  }
}
